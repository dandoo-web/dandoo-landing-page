...
const ResponsiveEarth = ({ onContactClick, setMousePosition, setIsHovered }) => {
  ...
  const scale =
    viewport.width < 4 ? 1 : viewport.width < 6 ? 1.5 : viewport.width < 8 ? 2 : 2.5;
...
const Scene = ({ onContactClick, setMousePosition, setIsHovered }) => {
  ...
  React.useEffect(() => {
    ...
    const cameraZ =
      viewport.width < 4 ? 8 : viewport.width < 6 ? 7 : viewport.width < 8 ? 6 : 5;
  ...
  return (
    <>
      ...
      {/* Lighting */}
      <ambientLight intensity={0.6} />
      <directionalLight intensity={1} position={[5, 5, 5]} color={0xffffff} />
      <pointLight position={[10, 10, 10]} intensity={0.5} />
...
const Contact = () => {
  ...
  return (
    ...
    >
      <p className="text-center font-medium pb-5 relative z-10 text-white">
        [ CONTACT US ]
      </p>
