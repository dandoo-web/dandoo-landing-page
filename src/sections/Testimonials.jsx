...
const Testimonials = () => {
  ...
  if (footerRef.current && testimonialsRef.current) {
    ...
    // Push footer down initially
    gsap.set(footer, { y: 100, opacity: 0 });

    ScrollTrigger.create({
      trigger: footer,
      start: "top 100%", // when the footer enters 95% of viewport height
      end: "top 65%",   // more scroll control
      scrub: true,
      onEnter: () => {
        gsap.to(footer, {
          y: 0,
  ...
  return (
    <>
      <div
        ref={testimonialsRef}
        className="relative w-full min-h-screen bg-black py-12 md:py-20 pb-32 md:pb-40 overflow-hidden"
      >
          ...
          <div className="text-center mt-16 md:mt-20 px-4">
            <div className="bg-white/5 backdrop-blur-lg border border-white/10 rounded-xl md:rounded-2xl p-6 sm:p-8 max-w-2xl mx-auto">
              ...
              <p className="text-white/70 mb-4 md:mb-6 text-sm sm:text-base leading-relaxed">
                Let's create something amazing together and make your project
      ...
      {/* 👇 Footer now scrolls into view naturally */}
